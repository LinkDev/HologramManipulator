...
namespace LinkDev.HologramManipulator
{
    ...
    public class BoundaryBox
    {
        ...
        public void UpdateEdgePoints()
        {
            ...
            Vector3 colliderWorldSize = Vector3.Scale(Size, Target.localScale);
            float minExtends = -1;
            switch (HologramType)
            {
                case HologramType._2D:
                    ...
                    float[] extendSize = new float[] { colliderWorldSize.x, colliderWorldSize.y, colliderWorldSize.z };
                    minExtends = extendSize.OrderBy((x => x)).ElementAt(1);
                    break;
                case HologramType._3D:
                    minExtends = Mathf.Min(Mathf.Abs(colliderWorldSize.x), Mathf.Abs(colliderWorldSize.y), Mathf.Abs(colliderWorldSize.z));
                    break;
            }
            ...
            m_ScaleFactor = (minExtends / extendFactor < minScaleFactor) ? minScaleFactor : minExtends / extendFactor;
            ...
            for (int i = 0; i < EdgePointsPosition.Length; i++)
            {
                ...
                Vector3 controllerPositionLocal = boxColliderCenter + controllerExtends;
